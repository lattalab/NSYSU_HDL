...
module IF_stage(
  ...
  input wire [`HWORD_SIZE-1:0] instruction, 
...
module ID_stage (
    ...
    input [`WORD_SIZE*`REG_FILE_SIZE-1:0] R_arr,
    ...
    always @(*) begin
	    for (i = 0; i < `REG_FILE_SIZE; i = i+1)
            R[i] = R_arr[`WORD_SIZE*i +: `WORD_SIZE];
    end
      ...
      if (~reset_n) begin
        ...
        if (branch_taken) begin
          ...
          case (ID_opcode)  // 1st part of EX operations
              ...
              `ADD_5: EX_ALU_out <= (ID_PC & 33'hfffffffc) + {ID_imm_offset[7:0], 2'b00}; // 32 = `WORD_SIZE
              ...
              `ASR_1: begin
                  if (ID_imm_offset[4:0] == 0) begin
                    if (ID_Rm_Rs[`WORD_SIZE-1] == 0)
                      ...
                      ALU_out = 33'h1ffffffff; //(`WORD_SIZE) = 32
                  end
                  else begin  // imm != 0
            ...
            `BL_BLX_H11: begin
              ...
              LR <= ALU_out[`WORD_SIZE-1:0] | 32'h00000001;
...
// MODULE: Write Back Stage for THUMB microprocessor: thumb_wb.v
module wb_stage (
    input wire reset_n,
    input wire clk,
    ...
    output reg [`WORD_SIZE*`REG_FILE_SIZE-1:0] R_arr
  );

    integer i, wb_i, found_wb_i;
    reg [`WORD_SIZE-1:0] R [`REG_FILE_SIZE-1:0];

    always @(*)begin
		for (i = 0; i < `REG_FILE_SIZE; i = i+1)
			R_arr[`WORD_SIZE*i +: `WORD_SIZE] <= R[i];
    end
...
module thumb (read_instruction_n, instruction_address,
    ...
    //wire [`HWORD_SIZE-1:0] IR;       // instruction register
    wire [`WORD_SIZE*`REG_FILE_SIZE-1:0] R;  // general regs
    ...
    wire [5:0] ID_opcode, EX_opcode; // internal Thumb opcode
