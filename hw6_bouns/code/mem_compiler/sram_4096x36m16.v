...
//       
//       Creation Date:  Mon Dec 30 04:07:49 2024
//       Version: 	r11p2
//
...
`ifdef POWER_PINS
module sram_4096x36m16 (VDDCE, VDDPE, VSSE, CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A,
    D, EMA, EMAW, EMAS, TEN, BEN, TCEN, TWEN, TA, TD, TQ, RET1N, STOV);
`else
module sram_4096x36m16 (CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS,
...
`endif
  ...
  reg NOT_TA5, NOT_TA4, NOT_TA3, NOT_TA2, NOT_TA1, NOT_TA0, NOT_TD35, NOT_TD34, NOT_TD33;
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_DP DY_ = RET1N_ ? (TEN_ ? D_ : TD_) : {36{1'bx}};
  assign `ARM_UD_SEQ Q_ = RET1N_ ? (BEN_ ? ((STOV_ ? (Q_int_delayed) : (Q_int))) : TQ_) : {36{1'bx}};
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     === 1'bx) begin
      ...
      Q_int = WEN_int !== 1'b1 ? D_int : {36{1'bx}};
      Q_int_delayed = WEN_int !== 1'b1 ? D_int : {36{1'bx}};
      ...
      if (WEN_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[35], 15'b000000000000000, writeEnable[34],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, D_int[35], 15'b000000000000000, D_int[34],
          15'b000000000000000, D_int[33], 15'b000000000000000, D_int[32], 15'b000000000000000, D_int[31],
          15'b000000000000000, D_int[30], 15'b000000000000000, D_int[29], 15'b000000000000000, D_int[28],
          15'b000000000000000, D_int[27], 15'b000000000000000, D_int[26], 15'b000000000000000, D_int[25],
          15'b000000000000000, D_int[24], 15'b000000000000000, D_int[23], 15'b000000000000000, D_int[22],
          15'b000000000000000, D_int[21], 15'b000000000000000, D_int[20], 15'b000000000000000, D_int[19],
          15'b000000000000000, D_int[18], 15'b000000000000000, D_int[17], 15'b000000000000000, D_int[16],
          15'b000000000000000, D_int[15], 15'b000000000000000, D_int[14], 15'b000000000000000, D_int[13],
          15'b000000000000000, D_int[12], 15'b000000000000000, D_int[11], 15'b000000000000000, D_int[10],
          15'b000000000000000, D_int[9], 15'b000000000000000, D_int[8], 15'b000000000000000, D_int[7],
          15'b000000000000000, D_int[6], 15'b000000000000000, D_int[5], 15'b000000000000000, D_int[4],
          15'b000000000000000, D_int[3], 15'b000000000000000, D_int[2], 15'b000000000000000, D_int[1],
          15'b000000000000000, D_int[0]} << mux_address);
        row = (row & ~row_mask) | (row_mask & (~row_mask | new_data));
        mem[row_address] = row;
      end else begin
...
`endif
 ...
 wire  CLK_;
  ...
  buf B140(EMAW_[0], EMAW[0]);
  buf B141(EMAW_[1], EMAW[1]);
  buf B142(EMAS_, EMAS);
  buf B143(TEN_, TEN);
  buf B144(BEN_, BEN);
  buf B145(TCEN_, TCEN);
  ...
  buf B155(TA_[8], TA[8]);
  buf B156(TA_[9], TA[9]);
  buf B157(TA_[10], TA[10]);
  buf B158(TA_[11], TA[11]);
  buf B159(TD_[0], TD[0]);
  buf B160(TD_[1], TD[1]);
  buf B161(TD_[2], TD[2]);
  buf B162(TD_[3], TD[3]);
  ...
  buf B167(TD_[8], TD[8]);
  buf B168(TD_[9], TD[9]);
  buf B169(TD_[10], TD[10]);
  buf B170(TD_[11], TD[11]);
  buf B171(TD_[12], TD[12]);
  buf B172(TD_[13], TD[13]);
  buf B173(TD_[14], TD[14]);
  buf B174(TD_[15], TD[15]);
  buf B175(TD_[16], TD[16]);
  buf B176(TD_[17], TD[17]);
  buf B177(TD_[18], TD[18]);
  buf B178(TD_[19], TD[19]);
  buf B179(TD_[20], TD[20]);
  buf B180(TD_[21], TD[21]);
  buf B181(TD_[22], TD[22]);
  buf B182(TD_[23], TD[23]);
  buf B183(TD_[24], TD[24]);
  buf B184(TD_[25], TD[25]);
  buf B185(TD_[26], TD[26]);
  buf B186(TD_[27], TD[27]);
  buf B187(TD_[28], TD[28]);
  buf B188(TD_[29], TD[29]);
  buf B189(TD_[30], TD[30]);
  buf B190(TD_[31], TD[31]);
  buf B191(TD_[32], TD[32]);
  buf B192(TD_[33], TD[33]);
  buf B193(TD_[34], TD[34]);
  buf B194(TD_[35], TD[35]);
  buf B195(TQ_[0], TQ[0]);
  buf B196(TQ_[1], TQ[1]);
  buf B197(TQ_[2], TQ[2]);
  buf B198(TQ_[3], TQ[3]);
  ...
  buf B203(TQ_[8], TQ[8]);
  buf B204(TQ_[9], TQ[9]);
  buf B205(TQ_[10], TQ[10]);
  buf B206(TQ_[11], TQ[11]);
  buf B207(TQ_[12], TQ[12]);
  buf B208(TQ_[13], TQ[13]);
  buf B209(TQ_[14], TQ[14]);
  buf B210(TQ_[15], TQ[15]);
  buf B211(TQ_[16], TQ[16]);
  buf B212(TQ_[17], TQ[17]);
  buf B213(TQ_[18], TQ[18]);
  buf B214(TQ_[19], TQ[19]);
  buf B215(TQ_[20], TQ[20]);
  buf B216(TQ_[21], TQ[21]);
  buf B217(TQ_[22], TQ[22]);
  buf B218(TQ_[23], TQ[23]);
  buf B219(TQ_[24], TQ[24]);
  buf B220(TQ_[25], TQ[25]);
  buf B221(TQ_[26], TQ[26]);
  buf B222(TQ_[27], TQ[27]);
  buf B223(TQ_[28], TQ[28]);
  buf B224(TQ_[29], TQ[29]);
  buf B225(TQ_[30], TQ[30]);
  buf B226(TQ_[31], TQ[31]);
  buf B227(TQ_[32], TQ[32]);
  buf B228(TQ_[33], TQ[33]);
  buf B229(TQ_[34], TQ[34]);
  buf B230(TQ_[35], TQ[35]);
  buf B231(RET1N_, RET1N);
  buf B232(STOV_, STOV);

  assign CENY_ = RET1N_ ? (TEN_ ? CEN_ : TCEN_) : 1'bx;
  assign WENY_ = RET1N_ ? (TEN_ ? WEN_ : TWEN_) : 1'bx;
  assign AY_ = RET1N_ ? (TEN_ ? A_ : TA_) : {12{1'bx}};
  assign DY_ = RET1N_ ? (TEN_ ? D_ : TD_) : {36{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_4096x36m16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .WEN(WEN_int), .Q_in(Q_int));
  `else
  assign Q_ = RET1N_ ? (BEN_ ? ((STOV_ ? (Q_int_delayed) : (Q_int))) : TQ_) : {36{1'bx}};
  `endif
...
`endif
`timescale 1ns/1ps
module sram_4096x36m16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [35:0] Q_out;
   input [35:0] Q_in;
   input CLK;
   input [11:0] A;
